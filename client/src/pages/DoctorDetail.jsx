      ...
      <div className="card mb-6">
        <div className="flex flex-col sm:flex-row gap-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-500 text-sm">{department?.name}</p>
          ...
          <Link to="/book-appointment" className="btn-primary self-start">
            Book Appointment
          </Link>
        </div>
      </div>
          ...
          <ul className="space-y-3">
            {qualifications.map((q, i) => (
              <li key={i} className="flex items-start gap-3 text-sm">
                <span className="bg-primary-100 text-primary-700 font-semibold px-2 py-0.5 rounded text-xs mt-0.5">
                  {q.year}
                </span>
                <div>
                  <p className="font-medium text-gray-900">{q.degree}</p>
                  <p className="text-gray-500">{q.institution}</p>
                </div>
              </li>
            ))}
          </ul>
